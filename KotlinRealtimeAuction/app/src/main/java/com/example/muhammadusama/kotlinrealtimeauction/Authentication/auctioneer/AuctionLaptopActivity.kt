...
import com.example.muhammadusama.kotlinrealtimeauction.R.string.camera
import com.example.muhammadusama.kotlinrealtimeauction.R.string.version
...
class AuctionLaptopActivity : AppCompatActivity(), View.OnClickListener , DatePickerDialog.OnDateSetListener
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mItemsDatabaseReference = mFirebaseDatabase?.getReference()?.child("Laptop")
    ...
    private fun post() {
        ...
        try {
            ...
            if (calendar?.timeInMillis!! < Calendar.getInstance().timeInMillis && !date?.after(Date())!!) {
                ...
                Toast.makeText(this@AuctionLaptopActivity, "End: $auctionEndTime", Toast.LENGTH_SHORT).show()

                val key = mItemsDatabaseReference?.push()?.key.toString()
                var laptop = Laptop(userId!!,key,ram, memory, model, processor, manufacturer, imageUrl,
                        auctionStartTime,auctionEndTime,auctionDate,minimumBid,"UNSOLD")

                mItemsDatabaseReference?.child(key)?.setValue(laptop)
    ...
    private fun durationDialog() {
        ...
        durationSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            ...
            override fun onItemSelected(parent: AdapterView<*>, view: View, position: Int, id: Long) {
                ...
                Toast.makeText(this@AuctionLaptopActivity, "Booking Duration : $auctionDuration Hour(s)", Toast.LENGTH_SHORT).show()

            }

            override fun onNothingSelected(parent: AdapterView<*>) {

                Toast.makeText(this@AuctionLaptopActivity, "Select the Duration!", Toast.LENGTH_SHORT).show()
            }
        }
